...
final class GenerateIssueWorkflow extends WorkflowAbstract
{
    ...
    private function draftPrompt(): string
    {
        ...
        return <<<PROMPT
            ...
            - GRANULARITY — decompose the work into SEVERAL focused steps, do NOT cram everything into one big step. Two reasons: (1) cost — each step's `ai()` starts with a fresh, lean context, whereas one fat step accumulates a huge history that is re-sent every turn (much more expensive); (2) checkability — a small step is a unit you can put a critic on, or whose artifact a later step verifies. Follow the recipe's phases as SEPARATE steps (e.g. design / implement / test, not one "do it all" step), and split a phase further when a part deserves its own validation (e.g. one implement step per method). BUT keep the BALANCE: a step must be a COHERENT, self-contained unit of real work that produces something validatable (an artifact, a passing gate) — do not over-split into trivial fragments that only add handoff overhead. Aim for a handful of meaningful, individually-verifiable steps, not one giant step and not twenty tiny ones.
            ...
            - PARAMS — a THIRD, optional channel for passing a CONCRETE VALUE (not content) to later steps to use IN CODE: `\$this->setParam('<name>', \$value)` pins it, a later step reads it with `\$this->param('<name>')`. Use it when a step DECIDES an exact value the code of a later step needs deterministically — a file path, a count, an id, a chosen strategy flag — rather than prose for the model. It is DURABLE (saved with the snapshot, survives a resume) and entirely OPTIONAL: most steps pass nothing this way, and that is fine — only reach for it when a later step's PHP genuinely needs a value an earlier step computed. (artifact = content for the model/critic; handoff = a prose note for the next model; param = a concrete value for the code.)
