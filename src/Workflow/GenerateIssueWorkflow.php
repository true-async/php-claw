...
final class GenerateIssueWorkflow extends WorkflowAbstract
{
    ...
    private function draftPrompt(): string
    {
        ...
        return <<<PROMPT
            ...
            - PASS THE BATON: a step's RETURN VALUE is its handoff to the next step — the engine feeds it into the next step's model automatically (no tool, no recall needed). So make every step `: string` and RETURN a concise summary of what it did plus the findings the next step must watch for (decisions, paths touched, what remains, gotchas). Write the step's prompt to end by producing that summary, and `return` it. This is the selective context relay — not the whole prior context, only what needs attention; the next step sees it as "the previous step handed this to you" at the top of its context
