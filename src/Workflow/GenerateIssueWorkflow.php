...
final class GenerateIssueWorkflow extends WorkflowAbstract
{
    ...
    private function draftPrompt(): string
    {
        ...
        return <<<PROMPT
            ...
            - PASS THE BATON: end EVERY step by calling the `handoff` tool with a concise `summary` of what it did and the `findings` the next step must pay attention to (decisions, paths touched, what remains, gotchas). Begin the NEXT step by reading it: tell that step's prompt to "first call recall(what='handoff')". This is the selective context relay — not the whole prior context, only what needs attention — so write each step's prompt to (1) recall the handoff, then (2) do its work, then (3) hand off to the next
