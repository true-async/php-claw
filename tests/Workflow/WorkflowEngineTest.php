...
use Claw\Workflow\CriticInterface;
use Claw\Workflow\Score;
use Claw\Workflow\Step;
use Claw\Workflow\StepExecutorInterface;
use Claw\Workflow\StepOutcome;
use Claw\Workflow\StepSpec;
use Claw\Workflow\ThresholdRetryPolicy;
...
final class WorkflowEngineTest
{
    ...
    public function retriesThenEscalatesAfterMaxAttempts(): void
    {
        ...
        $engine = new WorkflowEngine($this->executor(), $critic, new ThresholdRetryPolicy(passThreshold: 3, maxAttempts: 2));
    ...
    private function executor(): StepExecutorInterface
    {
        return new class () implements StepExecutorInterface {
    ...
    private function critic(array $scores): CriticInterface
    {
        return new class ($scores) implements CriticInterface {
            /** @param list<Score> $scores */
