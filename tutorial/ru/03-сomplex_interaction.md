...
Чтобы настроить бота, создайте его через `@BotFather` и получите токен. Затем
узнайте свой Telegram user id (например, через `@userinfobot`) и задайте три
...
Работает это так. `Session` и `Turn-Loop` не знают, откуда пришло сообщение — они
общаются через интерфейсы `ChatInterface` и `ConversationInterface`, поэтому новый
канал это просто их реализация. Бот опрашивает Telegram (long-poll); под
`TrueAsync` запрос паркует корутину и не блокирует поток. Каждое сообщение
направляется по `chat_id` в свой диалог, и на каждый чат запускается отдельная
`Session` со своей историей.

Бот публичен — написать может кто угодно, поэтому всё держится на списке доступа.
Каждое входящее событие (и сообщение, и нажатие кнопки) проверяется по `from.id`
против `CLAW_ALLOWED_CHATS`; чужие тихо игнорируются — молчание не выдаёт, что бот
существует. Список не может быть пустым: иначе запуск падает сразу, чтобы нельзя
было случайно открыть бота всем.
