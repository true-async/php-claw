...
Следующая проблема: поле ввода блокируется, пока агент обрабатывает запрос.
Чтобы решить эту проблему, нам нужно сделать поле ввода асинхронным.
Для этого напишем отдельную функцию `readLoop`, задача которой будет считывание ввода пользователя.
