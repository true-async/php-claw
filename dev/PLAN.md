...
Updated: 2026-08-16 · Active: S6, which runs before S2 (S1's remainder waits on Sage)
...
## S6 — Rooms without WebSocket

Upstream, in `/home/edmond/true-async-server`. Numbered S6 because a stage number
is a name, not a position: this stage runs HERE, before S2. It is work on the
files S1 has just been through, and every line of product code that rides them
makes the untangling dearer.

Goal: the pub/sub core stops knowing what a WebSocket is; a connection becomes
one kind of receiver among others.
Done when: no file of the core includes `ws_session.h` or tests
`HAVE_HTTP_SERVER_WEBSOCKET`, and a build configured with `--disable-websocket`
serves rooms — a publish in one thread reaching a `recv()` in another.

Sage 2026-08-15, the decision this stage executes, given the ask "separate rooms
from WebSocket 100%": the dependency is INVERTED rather than the mechanisms
split. Splitting them was rejected on one measured ground — the name space stays
ONE per server, so a room's publish still reaches a WebSocket subscriber of the
same name with no bridge and no copy, which is the live-board path php-claw runs
on today (`project/{key}/issues`). "A room never uses wildcards" is a limit of
Room's PHP contract, not of the index. What stays shared — the worker slot table,
the mailboxes, the one refcounted body per publish, the interest filter — is
shared because a receiver belongs to a thread, which is a property of the
threading model and not of WebSocket. Final.

The shape it lands in: `room_hub` (slots, mailboxes, fan-out, reliable send,
ring receiver, stats — today's topic_hub, algorithm untouched), `room_tree` (the
filter tree over a generic `room_receiver_t` carrying `ops->deliver`, embedded in
both a session and a ring consumer), `php_room.c` (the Room class, out of
http_server_class.c), and a WebSocket ADAPTER behind the build flag that
implements `deliver` and subscribes through the hub like anyone else. Calls go one
way only: PHP classes → hub → this thread's tree → the receiver's ops.

- [x] S6.1 A session subscribes through the hub
      done: `topic_hub_tree()` is gone from the public header and grep finds no
        caller outside the hub; the suite and the leak lane stay clean
      tier: T2 · role: Critic
      Five call sites (php_websocket.c and ws_session.c) and no lifetime change.
        This is the cycle the architecture pass found: a session subscribes by
        taking the tree out of the hub, so the tree has to call back into the hub,
        and ws_topic_tree.h declares three functions topic_hub.c implements.
      Measured against that description: the step removes the hub -> tree HANDOUT,
        and the back-edges stay. ws_topic_tree.c still calls
        ws_interest_publish(tree->hub, ...) on every subscribe and unsubscribe (4
        sites), and ws_topic_tree.h still declares the three ws_server_sub_*
        functions topic_hub.c implements. Both belong to S6.2, where the receiver
        becomes one kind — the cycle is not closed by this step.
      Critic 2026-08-16: no behaviour change a caller can observe, verified on
        every path — the id is spent at the same point, the at-cap id spend is
        unreachable (reaching the cap needs a live subscription, which means ws_id
        is already set), and the bailout teardown still reaches
        ws_topic_unsubscribe_all with a NULL tree. Four contract comments were
        false or misleading and were rewritten: unsubscribe on a detached thread
        leaves the entry in the session's own list, where getTopics() still reports
        it; unsubscribe_all runs on EVERY close and does its real work with a live
        tree; AT_CAP also covers a malformed filter, so the precondition is now
        stated; and a session, unlike a server receiver, never causes an attach.
        Also taken: the block moved next to the server-subscriber section it will
        merge with, and ws_topic_tree.h's "called from ws_session_destroy" was
        corrected to name its new caller.
      Critic 2026-08-16, recorded rather than fixed: topic_hub.c names ws_session_t
        for the first time (0 references at HEAD, 3 now, plus a session->ws_id
        read) — the direction S6 is undoing. Kept because the id handout is where
        S6.2 puts it anyway and the type is what S6.2 replaces; the cost is that
        S6.2 rewrites these three functions, which is why it is written down here.
      handoff: topic_hub_session_subscribe/unsubscribe/unsubscribe_all in
        topic_hub.c, beside the server-subscriber block they merge with in S6.2;
        the two refusals a caller acts on are a status enum, not an inferred NULL.
        topic_hub_next_id is static now. true-async/server PR #162, all 9 checks
        green. Measured: tests/phpt 380/1 (the pre-existing h3 failure), the leak
        lane 9/0, fuzz_ws_topic 570k runs clean.
      Fixed on the way (rule 3): fuzz_ws_topic had not compiled since S1.10 gave
        ws_topic_publish and ws_server_sub_try_deliver the shared-body scratch —
        CI built neither WebSocket fuzz target, so a whole step passed over it.
        Both are built and run in the embedded fuzz stage now (the frame target
        at 30 s: it executes about one case a second).
- [x] S6.2 One kind of subscriber in a node
      done: ws_topic_tree.h names no `ws_session_t` and declares nothing the hub
        implements; the tagged `{ptr, kind}` reference and the branch on kind are
        gone; tests 038 (cross-worker) and the room tests stay green over repeated
        runs
      tier: T2 · role: Critic
      Sage: the filter list moves onto the receiver (server subscriptions
        included, so an unsubscribe stops re-finding its node by filter), and every
        receiver takes an id from the hub's counter — without that, the unified
        "do not send to the sender" check would skip server receivers at
        except_id 0. The park machinery is NOT touched by this step.
      Found while building it: the filter list arriving on a server subscription
        made topic_hub_detach_ex leak it — the loop NULLs sub->local first, and
        topic_hub_unsubscribe then skips its tree work, so neither branch freed
        the list. The leak lane cannot see it: it excludes the two tests that
        reach a dead-request teardown with a live subscription.
      Sage (Fable) 2026-08-16, called by Edmond on the code rather than on a
        dispute: no correctness defect; fit to merge. The detach order verified
        against the mailbox drain, the retired interest counters and
        ws_topic_tree_free, and the tree-less arm judged the less brittle of the
        two (the full arm would be a no-op anyway, because the attachment is
        unlinked before the loop — which the NULL arm does not depend on).
        Required before merge: the id invariant is held by convention alone (both
        hub entry points assign before subscribing), and its failure is silent —
        a receiver with id 0 matches except_id 0 and is skipped by every publish,
        which is exactly the Room path. Taken: ZEND_ASSERT in ws_topic_subscribe.
        Also taken: the receiver's zero-initialisation is a contract and was
        undocumented, and the fuzz harness claimed tombstone-and-compact coverage
        it did not have — one receiver now unsubscribes itself from inside the
        delivery callback, which is the socket-write-closes-its-own-connection
        case. Final.
      handoff: ws_topic_receiver_t {ops, id, mark, filters} in ws_topic_tree.h,
        embedded in ws_session_t and ws_server_sub_t and reached by offsetof; the
        node array is ws_topic_receiver_t ** with NULL as the tombstone. The hub's
        session calls are topic_hub_receiver_*. true-async/server PR #163, all 9
        checks green — LINUX_X64_DEBUG_ZTS is the one that exercises the new
        assert. Measured: tests/phpt 380/1, websocket 77/77 three runs, the leak
        lane 9/0, fuzz_ws_topic 522k runs clean with the self-closing receiver.
- [ ] S6.3 The core moves to its own name
      done: `git mv` to src/room/ and include/room/ with `room_*` prefixes, grep
        for "websocket" over those files is empty, and the diff is mechanical
      tier: T2 · role: —
      PHP-visible names do not move: the classes, the `setWs*` config, the `ws_*`
        keys in getRuntimeStats and the test hook are a contract php-claw and the
        tests depend on.
- [ ] S6.4 Rooms build without WebSocket
      done: a build configured with --disable-websocket compiles and passes a
        publish-here / recv-there probe across two threads; the build with
        WebSocket keeps the suite and the lane clean
      tier: T2 · role: Critic
      The no-op macro block in http_server_class.c and the guards around Room go;
        only the adapter and WebSocket itself stay behind the flag.
- [ ] S6.5 The contracts stop saying WebSocket
      done: stubs and header prologues describe a subscriber as "a WebSocket
        connection or a server-side receiver", and the regenerated stubs pass
      tier: T1 · role: —

Risk, named by Sage where a fault would be silent rather than loud: an interest
filter counter that drops too low loses a cross-thread message and the tests catch
it only by chance (run 038 in series); the tombstone invariant, where a decrement
under a walk turns a settle into a use-after-free; the +1/-1 discipline on the
message body under the new deliver path, visible only as `ws_bodies` against
`ws_bodies_freed`; and the id handout above.

Not in this stage, and Sage's ruling on it: run control does not belong on a room.
Its contract is one addressed reader, no loss, with acknowledgement, and a room is
a broadcast with a drop-oldest ring. After S6.4 that becomes its own primitive of
the room core — a channel addressed by receiver rather than by name, on the same
slots, mailboxes and parking, outside the topic tree. Until it exists,
`run/{id}/control` stays on a room, where `send()` is acknowledged by the target
worker's mailbox and loss is visible through `lostCount()` / `ws_sub_overflow`.
